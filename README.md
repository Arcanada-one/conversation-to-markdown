...
- Re-exporting a conversation **overwrites** the file of the same name. Earlier versions of this document claimed Chrome appends `(1)`, `(2)`, … and preserved the old file; that was wrong — the extension asks for `overwrite` deliberately, because a folder filling with `(1)`, `(2)`, `(3)` copies of the same unchanged conversation is worse, and resume matches exact filenames.
- A conversation that **gained messages** since you exported it is the exception: it is written as a **new dated copy** beside the old file, which is left untouched. The run reports how many were saved that way. Conversations that have not changed are skipped without being re-read.
