...
Two failures that no fixture could have caught, both found by driving the real
extension in a real browser.

### The extension assumed everyone's ChatGPT is in English

Three of the strings it matched are **translated UI text**, and all three decided
control flow. Measured on the same account and the same conversations, changing
only the browser locale:

| matched string        | English UI            | Russian UI            |
|-----------------------|-----------------------|-----------------------|
| pagination row        | `Show more`           | `Показать больше`     |
| project row suffix    | `, chat in project X` | `, чат в проекте X`   |
| artefact download     | `Download file`       | `Скачать файл`        |

There is no account setting for this — the interface follows the **browser**, so
a non-English user got different behaviour from the same extension.

- **A Project's conversation list stopped short and still said "complete".**
  Projects paginate by a click, and the control was matched by its English label.
  With a translated interface it was invisible, and "no control standing" is the
  very evidence used to conclude the list was fully read. Worst case: a Project
  exported partially, reported as complete.
- **The project name was baked into every folder name.** The accessibility suffix
  was stripped by matching English words, so a Russian interface left
  `, чат в проекте Aether` in the title, the filename, and the folder name. The
  fallback path failed in the same locale for the same reason.

Both are now identified by page **structure** instead of by wording, so no
language is involved. The pagination tests run in English, Russian and German —
German was never measured, and is there to prove the detector never consults
language at all.

### Files generated by ChatGPT were not exported at all

A conversation containing five generated PDF/DOCX documents exported **none** of
them, and reported success. Files that ChatGPT creates for you render in a
separate panel with no link, no download attribute, and no identifying markup;
they are also outside the message stream, so scanning each message can never find
them. Every attachment selector the extension had returned nothing.

Such files are now retrieved through ChatGPT's own conversation data, the same way
the page itself does it, and appear under a **Files** heading in the exported
Markdown. If a file cannot be retrieved, it is **named in the export** along with
the reason — a missing file is never silently dropped, because that turns an
incomplete export into a clean-looking one.

### Fixed

- Project conversation lists no longer stop at the first page under a non-English
  interface, and a list that cannot be confirmed complete is not reported as
  complete.
- Conversation titles no longer carry the project name into filenames and folder
  names.
- A title that the sidebar truncates is no longer cut short in the folder name.
- Generated (code-interpreter) documents are exported, not skipped.
- An unreadable conversation API is reported instead of being treated as
